...
let main = () => {
	...
	let userdataControl = {
				...
				if (check()) {
					...
					content += `
						<div class="flex flex_column" style="gap: 5px; width: 300px">
							<h4 style="width: 300px">Бәлкім, сіз <span style="font-weight: bold">«${u.profile.location[1]}»</span> сабағын жабуды ұмыттыңыз.</h4>
							<button class="bobatron button_go" Bt-CM="0.7" id="location_issues_goback" style="width: 100%;">Артқа<div class="arrow_right_b"></div></button>
						</div>
						<div class="flex flex_column" style="gap: 5px; width: 300px">
							<h4 style="width: 300px">Бір уақытта бірнеше Микролаба қойындысын ашу ұсынылмайды.</h4>
							<button class="bobatron button_warning" Bt-CM="0.7" id="location_issues_ignore" style="width: 100%;">Елемеу<div class="arrow_right_b"></div></button>
						</div>
					`
					...
					content += `
						<div class="flex flex_column" style="gap: 5px; width: 300px">
							<h4 style="width: 300px">Бәлкім, <span style="font-weight: bold">«${lessonLocation[1]}»</span> сабағы сізге әлі қолжетімді емес.</h4>
							<button class="bobatron button_go" Bt-CM="0.7" id="location_issues_goback" style="width: 100%;">Басты бет<div class="arrow_right_b"></div></button>
						</div>
					`
		...
		alright: (c) => {
			...
			bobatron.scanner()
			document.title = `SandyqBilim | ${contents[lessonNameL[1]].header}`
			document.getElementById("body").style.overflow = "hidden"
		},
	...
	let testHandler = {
		...
		testHeader: (num) => {
			...
			header.innerHTML = `
			<h4>Сұрақ ${num + 1}</h4>
			<h4>${testHandler.lastCount()}/${tasks[lessonNameL[1]].amount} аяқталды</h4>`
		},
		testRender: (num) => {
			...
			<button id="test_check_button" class="bobatron" Bt-CM="0.7" style="opacity: 0; pointer-events: none">Тексеру</button>
			`
		...
		testNavigator: (num) => {
			...
			<div class="flex flex_gap_10" style="width: 100%; justify-content: space-between">
				...
				<button class="bobatron button_go_lessonComplete" Bt-CM="0.7" id="test_navigator_end" style="width: fit-content">Аяқтау</button>
		...
		lessonComplete: () => {
			...
			if (lessList[lessList.indexOf(lessonNameL[1]) + 1]) {
				...
				<div class="flex flex_gap_10" style="width: 100%">
					...
					<button class="bobatron button_go_lessonComplete" Bt-CM="0.7" id="lesson_complete_go_next" style="width: 100%">Келесі<div class="arrow_right_b"></div></button>
				</div>
				<div class="flex flex_alignitems_center flex_gap_10" style="width: 100%">
					...
					<h4 style="width: 100%">Келесі<br><span style="font-weight: bold">${contents[b].header}</span></h4>
				</div>
				`
			}
			else {
				...
				<div class="flex flex_column" style="gap: 5px; width: 100%">
					<button class="bobatron button_go_lessonComplete" Bt-CM="0.7" id="lesson_complete_go_back" style="width: 100%; justify-content: flex-start"><div class="arrow_left_b"></div>Меню</button>
					<h4 style="width: 100%">Құттықтаймыз! Сіз осы курсты аяқтадыңыз.</h4>
				</div>
				`
			}
