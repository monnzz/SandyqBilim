let base = () => {	
	let onload = 1, lessonName = "menu", lessonLocation = [lessonName, ""], lessList = ["sabaq_1", "sabaq_2", "sabaq_3", "sabaq_4", "sabaq_5"]
	let userdata = {
		profile: {
			username: "Username",
			ava: "ava_1",
	...
	let interface = {
		...
		lessonStartButtons: (target, progress, progressMax) => {
			try {
				...
				if (progress == 0) { msg = "Бастау" }
				else if (progress == progressMax) { msg = "Қайта өту" }
				else if (progress != 0) { msg = "Жалғастыру" }
